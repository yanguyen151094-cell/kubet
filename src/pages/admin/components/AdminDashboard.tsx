...
export default function AdminDashboard() {
    ...
    try {
      ...
      if (result.success) {
        ...
        if ((result as { warning?: string }).warning) {
          setSaveError((result as { warning?: string }).warning ?? '');
        }
        setTimeout(() => { setSaved(false); setSaveError(''); }, 3000);
      } else {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
      {/* ===== CẢNH BÁO THIẾU SUPABASE ENV ===== */}
      {!isSupabaseReady && (
        <div className="fixed top-0 left-0 right-0 z-50 bg-red-500 text-white px-4 py-2 text-sm text-center">
          <strong>⚠ Cảnh báo:</strong> Thiếu biến môi trường Supabase (VITE_PUBLIC_SUPABASE_URL / VITE_PUBLIC_SUPABASE_ANON_KEY). 
          Dữ liệu sẽ chỉ lưu vào trình duyệt (localStorage). Vui lòng thêm env vars trong Vercel Dashboard.
        </div>
      )}
      {/* ===== END CẢNH BÁO ===== */}

      <div className={`${!isSupabaseReady ? 'pt-12' : ''} flex flex-col md:flex-row w-full min-h-screen`}>
        {/* Mobile header */}
